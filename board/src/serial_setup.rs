...
impl<T: Instance> UartePort<T> {
    pub fn new(serial: Uarte<T>) -> UartePort<T> {
        // We get warnings here about how this will be considered an error,
        // in later Rust versions.
        // Safety: We know that these static buffers are only accessed here
        // and the UART hardware ensures synchronized access.
        let (tx, rx) = serial
            .split(
                unsafe { &mut TX_BUF },
                unsafe { &mut RX_BUF }
